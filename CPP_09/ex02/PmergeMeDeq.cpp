...
*/
int PmergeMeDeq::binarySearch(const std::deque<int>& v, int value, int& cmpCount)
{
    int left = 0;
    int right = v.size() - 1;
...
*/
int PmergeMeDeq::computeUsableMainRange(int k, size_t mainChainSize, int blockSize)
{
...
*/
int	PmergeMeDeq::doInsertion(std::deque<int>& mainChain, std::deque<int>& pending, int blockSize, int k)
{
    int                 cmpCount = 0;
...
*/
int	PmergeMeDeq::setInsertionOrder(std::deque<int>& mainChain, std::deque<int>& pending, std::deque<size_t>& jacSequence, int blockSize, int extraBlocksOverJacobsthal)
{
	int		cmpCount = 0;
	size_t	currJac = 3;
	size_t	prevJac = 1;
...
/*
	...
	@return How many comparisons were done.
*/
int	PmergeMeDeq::insertPendingIntoMain(std::deque<int>& mainChain, std::deque<int>& pending, int blockSize) {

    int blockAmountInPending = pending.size() / blockSize; // How many blocks are in pending.
    int excessBlocks = 0;
    std::deque<size_t> jacSequence = jacobsthalIndices(blockAmountInPending + 1);
    size_t k = 1;
    int cmpCount = 0;
    size_t prevJac = 1;
...
void PmergeMeDeq::mergeInsertSortDeque(int argc, char **argv, int& numCmpdeq, std::deque<int>& deq)
{
    ...
    while (recLvl > 0) {
        ...
        if (!pending.empty())
            numCmpdeq += insertPendingIntoMain(mainChain, pending, blockSize); // Insert pending into main chain.
        if (!leftover.empty())
