...
/*
	...
	And so forth.
*/
std::deque<size_t> PmergeMeDeq::jacobsthalIndices(size_t n)
{
	std::deque<size_t> J;
	size_t j0 = 1, j1 = 1;
...
*/
int PmergeMeDeq::binarySearch(const std::deque<int> &mainVals, int value, int &cmpCount)
{
	int left = 0;
...
/*
	...
	This is done by:
	- Finding the pendigVal in pending deque.
...
*/
int PmergeMeDeq::findMaxMainIndex(int pendingVal, const std::deque<int> pending, std::deque<int> &mainChain, const std::deque<int> originalMain)
{
	size_t i = 0;
...
*/
int PmergeMeDeq::doInsertion(std::deque<int> &mainChain, const std::deque<int> pending, int blockSize, int k, const std::deque<int> originalMain)
{
...
*/
int PmergeMeDeq::setInsertionOrder(std::deque<int> &mainChain, const std::deque<int> pending, std::deque<size_t> &jacSequence, int blockSize, int extraBlocksOverJacobsthal, const std::deque<int> original)
{
	int cmpCount = 0;	// How many comparisons were done.
...
/*
	...
	@return How many comparisons were done.
*/
int PmergeMeDeq::insertPendingIntoMain(std::deque<int> &mainChain, const std::deque<int> pending, int blockSize, const std::deque<int> originalMain)
{
	...
	std::deque<size_t> jacSequence = jacobsthalIndices(blockAmountInPending); // Get the jacobsthal sequence (based on the amount of blocks in pending).
...
*/
int PmergeMeDeq::mergeInsertSortDequeRecursive(std::deque<int> &tmp, int recursionLvl, int &nmbCmpdeq)
{
	...
	return mergeInsertSortDequeRecursive(tmp, recursionLvl + 1, nmbCmpdeq);
}
...
*/
void PmergeMeDeq::mergeInsertSortDeque(int argc, char **argv, int &numCmpDeq, std::deque<int> &deq)
{
	// 1. Store input in deque
	storeInputDeq(argc, argv, deq);
	...
	// 3. Sort part 1
	int recLvl = mergeInsertSortDequeRecursive(deq, 1, numCmpDeq);
	...
	while (recLvl > 0)
	{
		...
		for (size_t i = 0; i < deq.size(); i++) // Sort sequence into mainchain, pending and leftover
		{
			int chain = organizeChains(i, blockSize, deq.size());
			if (chain == 0)
				mainChain.push_back(deq[i]);
			else if (chain == 1)
				pending.push_back(deq[i]);
			else
				leftover.push_back(deq[i]);
		}

		// Store the original main chain to keep track of pair positions.
		const std::deque<int> originalMain = mainChain;

		// Insertion
		if (!pending.empty())
			numCmpDeq += insertPendingIntoMain(mainChain, pending, blockSize, originalMain); // Insert pending into main chain.
		if (!leftover.empty())
			mainChain.insert(mainChain.end(), leftover.begin(), leftover.end()); // Append leftovers to main chain.

		deq = mainChain; // Reassign deq
