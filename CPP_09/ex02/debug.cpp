...
void debugPrintCurrentJacobsthal(const std::vector<int> &pending, int k, int blockId, int jacPairStart, int jacPairEnd)
{
	std::cout << "Current Jacobsthal: " << k << std::endl;
	std::cout << "At which pos does block b" << blockId + 1 << " start in pending?: " << jacPairStart << std::endl;
	std::cout << "At which pos does block b" << blockId + 1 << " end in pending?: " << jacPairEnd << std::endl;
	std::cout << "Which (last) value does block b" << blockId + 1 << " have in pending?: " << pending[jacPairEnd] << std::endl;
	std::cout << std::endl;
}
